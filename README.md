...
本仓库已转移至 [https://github.com/XidianSCgroup/TRUST](https://github.com/XidianSCgroup/TRUST)

基于门限 Paillier 加密算法（paililerTD, FastpaiTD），涵盖了论文 [POCF](https://ieeexplore.ieee.org/abstract/document/7500106)、[SOCI](https://ieeexplore.ieee.org/abstract/document/9908577)、[SOCI+](https://ieeexplore.ieee.org/abstract/document/10531248) 和 [TRUST](https://arxiv.org/abs/2412.01073) 协议的实现。
...
- #### **local_version**
    ...
    - `SEAT_C++`: mini-batch 梯度下降线性回归基于 `SEAT` 的 C++ 实现，支持 FastPaiTD 加密，密文版本
    - `SEAT_SGX`: mini-batch 梯度下降线性回归基于 `SEAT` 的 SGX 实现，支持 FastPaiTD 加密，密文版本
    - `POCF_C++`: `POCF` 协议的本地 C++ 可运行版本
    - `SOCI_C++`: `SOCI` 协议的本地 C++ 可运行版本
    - `TRUST_C++`: `SOCI+` 和 `TRUST` 协议的本地 C++ 可运行版本，相当于 `SOCI+_C++` 和`TRUST_C++` 的实现，涵盖对比方案
    - `TRUST_SGX`: `TRUST` 协议的本地 SGX 可运行版本
