...
namespace PHESPACE {
    ...
    public:
        ...
        Paillier(PublicKey pk, int sigma) {
            ...
            this->table = Pre_Compute::constructTable(pk.h_N, this->N_Square, pk.L_k); //真正花时间的地方是计算h_N
            Offline_Phase(sigma);
        ...
        void scl_mul(mpz_t dst, mpz_t c, long long cons);
    ...
    void Paillier::scl_mul(mpz_t dst, mpz_t c, long long cons) {
        mpz_t cons_mpz;
    ...
    void Paillier::Offline_Phase(int sigma) {
        ...
        mpz_set_si(zero, 0);
        mpz_set_si(one, 1);
        mpz_inits(offlineVal.e0, offlineVal.e1, NULL);
    ...
    void Paillier::encrypt(mpz_t dst, mpz_t m) {
        ...
        if (table != nullptr) {
            // printf("[CP] 加密过程-预计算分支!\n");
            mpz_t mask;
            mpz_init(mask);
            Pre_Compute::compute(mask, r, table, N_Square);//(h^N mod N^2)^r
            //Pre_Compute::fastPower(mask, pk.h_N, r, N_Square);
    ...
    void Paillier::decrypt(mpz_t dst, mpz_t c) {
        ...
        if (mpz_cmp_ui(sk.alpha, 0) == 0 || mpz_cmp_ui(sk.N, 0) == 0) {
            cerr << "Error: No private key" << endl;
            exit(1);
        }
    ...
    public:
        ...
        PaillierThirdParty *KGen(int k, int sigma) {
            ...
            mpz_init(y);
            while (true) {
                mpz_urandomb(y, randstate, mpz_sizeinbase(N, 2));
                mpz_t y_gcd_N;
                mpz_init(y_gcd_N);
                mpz_gcd(y_gcd_N, y, N);
                if (mpz_cmp(y, N) < 0 && mpz_cmp_ui(y_gcd_N, 1) == 0) {
                    break;
                }
            }
            ...
            mpz_powm(h_N, h, N, N_square); //真正花时间的地方是计算h_N
            ...
            PaillierThirdParty sk1 = PaillierThirdParty(N, alpha1);
            ...
            mpz_mul_ui(tmp, N, 2 * yita);
            mpz_add(alpha2, alpha2, tmp);
            PaillierThirdParty sk2 = PaillierThirdParty(N, alpha2);

            static PaillierThirdParty res[2] = {sk1, sk2};
        ...
        static bool isCoPrime(mpz_t p, mpz_t q, mpz_t p_another, mpz_t q_another) {
