...
static int sigma; // 安全参数比特位长
static int k; // 安全强度 , 64、80、104、112
...
//        gmp_randseed_ui(*randstate, (unsigned long int) seed);
    ...
    /**
     * 同态加法
     * @param dst 密文结果，即[x+y]，括号代表密文的意思
     * @param ex  密文x，记[x]
     * @param ey  密文y，记[y]
     */
    ...
    /**
     * 标量乘法
     * @param dst 密文结果，即[cons * c]，括号代表密文的意思，亦可理解成为[c+c+...+c]，一共加了cons遍
     * @param c  密文c，记[c]
     * @param cons 标量，记cons
     */
     ...
     * 门限解密
     * @param dst 明文结果
     * @param c1  部分解密密文c1，记[c1]
     * @param c2  部分解密密文c2，记[c2]
     */
