...
namespace PHESPACE
{
    ...
    public:
        ...
        PrivateKey()
        {
            mpz_inits(this->alpha, this->N, this->mid, NULL);
        }
    ...
    {
    public:
        mpz_t N, mid, h, h_N, L, N_Square;
        int L_k;

        PublicKey()
        {
            mpz_inits(this->N, this->mid, this->h, this->h_N, this->L, this->N_Square, NULL);
        }
    ...
    public:
        ...
        PaillierThirdParty *KGen(int k, int sigma)
        {
            ...
            while (true)
            {
                // mpz_urandomb(y, randstate, mpz_sizeinbase(N); //解决的bug之一
            ...
            //            mpz_t debug;
            //            mpz_init(debug);
            //            mpz_add(debug, alpha1, alpha2);
            //            mpz_mod(tmp, debug, two_alpha);
            //            gmp_printf("debug1 = %Zd\n", tmp);
            //            mpz_mod(tmp, debug, N);
            //            gmp_printf("debug2 = %Zd\n", tmp);
    ...
    void check_smul(int i, mpz_t eres, mpz_t m_1, mpz_t m_2, Paillier pai)
    {
        ...
        mpz_t res, xy;
        mpz_inits(res, xy, NULL);
        pai.decrypt(res, eres);
        gmp_printf("dec answer res = %Zd\n", res);
        mpz_mul(xy, m_1, m_2);
        if (mpz_cmp(res, xy) != 0)
        {
            gmp_printf("m_1 = %Zd\n", m_1);
